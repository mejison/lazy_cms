...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function order_by_field($field, $orderby, $direction = 'ASC')
	{
		...
		if (is_array($orderby))
		{
			...
			if (strpos($orderby, ',') !== FALSE)
			{
				...
				foreach (explode(',', $orderby) as $part)
				{
					$part = trim('"'.str_replace('"', '\"', trim($part)).'"');
					$temp[] = $part;
				}
			...
			{
				$orderby = trim('"'.str_replace('"', '\"', trim($orderby)).'"');
			}
			
		}
		
		$orderby_statement = 'FIELD ('.$field.', '.implode(', ',$orderby).') '.$direction." \r\n";
